...
export const processNpcUpdate = async (
    ...
    if (foundMatch) {
        ...
        if (tagParams.realm) {
            let newRealm = tagParams.realm;
            // More robust realm normalization.
            if (newRealm && effectiveRace && newKb.worldConfig?.isCultivationEnabled) {
                    ...
                    if (characterRealmProgression.length > 0) {
                        ...
                        for (const majorRealm of sortedRealms) {
                            ...
                            if (index !== -1) {
                                ...
                                if (cleanedRealm) {
                                    console.log(`[NPC_UPDATE REALM NORMALIZE] Cleaned realm from "${newRealm}" to "${cleanedRealm}" based on major realm "${majorRealm}".`);
