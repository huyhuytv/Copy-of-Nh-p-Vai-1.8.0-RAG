...
import { generateAuctionData, runAuctionTurn, runAuctioneerCallPrompt } from '../../services/geminiService';
...
export const useAuctionActions = ({
    ...
    const handlePlayerAuctionAction = useCallback(async (itemId: string, bidAmount: number) => {
        ...
        try {
            ...
            // If an NPC immediately counter-bids, reset timer
    ...
    const handleAuctioneerCall = useCallback(async () => {
        ...
        try {
            const { response, rawText } = await runAuctioneerCallPrompt(workingKb, currentItem, newCallCount, onPromptConstructedForLog);
            setReceivedEconomyResponsesLog(prev => [rawText, ...prev].slice(0,10));
