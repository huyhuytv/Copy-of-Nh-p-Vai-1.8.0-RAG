...
import { WorldSettings, DIALOGUE_MARKER, TU_CHAT_TIERS } from '../types';
...
export const continuePromptSystemRules = (worldConfig: WorldSettings | null, mainRealms: string[]): string => `
...
**36. Quy Tắc về Tù Nhân và Bạn Đồng Hành (CƠ CHẾ MỚI):**
    *   **Tình huống Bắt giữ:**
        *   **Sau chiến đấu:** Nếu có NPC đầu hàng (hệ thống sẽ thông báo), hãy mô tả cảnh bắt giữ và sử dụng tag \`[PRISONER_ADD: ...]\` để biến NPC đó thành tù nhân.
        *   **Trong lời kể:** Nếu người chơi thực hiện hành động bắt cóc hoặc bắt giữ thành công, hãy sử dụng tag \`[PRISONER_ADD: ...]\` để biến NPC mục tiêu thành tù nhân.
    *   **Tag [PRISONER_ADD: ...]:**
        *   Sử dụng để thêm một tù nhân mới. NPC gốc sẽ bị xóa khỏi danh sách NPC thông thường.
        *   **Thuộc tính bắt buộc:** \`name="Tên NPC"\`.
        *   **Thuộc tính tùy chọn (để AI thiết lập hợp lý):** \`willpower=80\`, \`resistance=90\`, \`obedience=10\`, \`affinity=-50\`, và các thuộc tính khác của NPC (description, gender, realm...).
        *   Ví dụ: \\\`[PRISONER_ADD: name="Tên Cướp", description="Một tên cướp hung hãn.", willpower=70, resistance=95, obedience=5, affinity=-60]\\\`
    *   **Thêm Đạo Lữ / Nô Lệ:**
        *   Khi người chơi kết hôn, hoặc mua/nhận được nô lệ thông qua diễn biến câu chuyện, hãy sử dụng các tag sau. NPC gốc sẽ bị xóa.
        *   **[WIFE_ADD: name="Tên", ... (thuộc tính NPC)]:** Thêm một Đạo Lữ mới. \`willpower\` và \`obedience\` thường sẽ cao.
        *   **[SLAVE_ADD: name="Tên", ... (thuộc tính NPC)]:** Thêm một Nô Lệ mới. \`obedience\` nên là 100, \`willpower\` có thể thấp hoặc cao.
        *   Ví dụ: \\\`[WIFE_ADD: name="Tiểu Long Nữ", affinity=100, willpower=100, obedience=100]\\\`
        *   Ví dụ: \\\`[SLAVE_ADD: name="A Châu", description="Một cô gái có thân thế bí ẩn, được bạn cứu về.", affinity=20, willpower=40, obedience=100]\\\`
    *   **Cập nhật chỉ số:** Để thay đổi các chỉ số của các thực thể này, hãy sử dụng các tag tương ứng:
        *   **[WIFE_UPDATE: name="Tên Đạo Lữ", newName="Tên Đạo Lữ Mới", affinity=+=GiáTrị, willpower=+=GiáTrị, obedience=+=GiáTrị]**
        *   **[SLAVE_UPDATE: name="Tên Nô Lệ", newName="Tên Nô Lệ Mới", affinity=+=GiáTrị, willpower=+=GiáTrị, obedience=+=GiáTrị]**
        *   **[PRISONER_UPDATE: name="Tên Tù Nhân", newName="Tên Tù Nhân Mới", affinity=+=GiáTrị, willpower=+-GiáTrị, resistance=+-GiáTrị, obedience=+=GiáTrị]**
    * Ví dụ: \\\`[PRISONER_UPDATE: name="Tên Tù Nhân", obedience="+=10", resistance="-=10", affinity="+=5"]\\\`
`;